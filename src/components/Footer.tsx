...
export default function Footer() {
  ...
  return (
    <footer style={{ background: 'linear-gradient(180deg, #033a9d 0%, #022d80 100%)' }} className="border-t border-white/10">
      {/* Top statement band */}
        ...
        <div className="section-container py-16 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-10">
          <div className="max-w-xl">
            ...
            <h3 className="font-inter-tight font-black text-white text-3xl md:text-4xl leading-tight">
              Insights for the <span style={{ color: '#36c5f0' }}>Modern Enterprise.</span>
            </h3>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3 w-full max-w-md">
            {submitted ? (
              <div className="flex-1 flex items-center justify-center gap-2 bg-white/10 border border-white/20 rounded-full px-6 py-3.5 text-[#36c5f0] text-sm font-semibold">
              ...
              <>
                <input
                  ...
                  className="flex-1 bg-white/10 border border-white/15 rounded-full px-5 py-3.5 text-white text-sm placeholder-white/30 focus:outline-none focus:border-[#36c5f0]/60 transition-colors"
                />
                <button
                  type="submit"
                  className="text-[#033a9d] font-semibold text-sm px-6 py-3.5 rounded-full transition-colors whitespace-nowrap"
                  style={{ background: 'linear-gradient(135deg, #36c5f0 0%, #007cf4 100%)' }}
                >
                  Subscribe
                </button>
              </>
      ...
      <div className="section-container py-16">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-10">
          ...
          {[['Solutions', solutions], ['Industries', industries], ['Company', company], ['Resources', resources], ['Legal', legal]].map(([title, links]) => (
            <div key={title as string}>
              <h4 className="text-white text-xs font-semibold uppercase tracking-widest mb-5">{title as string}</h4>
      ...
      <div className="border-t border-white/10">
        <div className="section-container py-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-xl mx-auto">
            {locations.map(loc => (
              <div key={loc.city} className="bg-white/10 border border-white/15 rounded-xl p-4 text-center">
                <div className="text-3xl mb-2">{loc.flag}</div>
                <div className="text-white font-semibold text-sm">{loc.city}</div>
                <div className="text-white/40 text-xs mt-0.5">{loc.detail}</div>
              </div>
            ))}
          </div>
        </div>
